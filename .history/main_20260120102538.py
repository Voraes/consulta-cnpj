...
def cache_valido(entry):
    ...
    return datetime.now(datetime.now) - atualizado < timedelta(days=CACHE_TTL_DAYS)


# ________Consulta________
...
# ________Pipeline principal________
...
def ler_cnpjs_arquivo(caminho):
    ...
    if not cnpjs:
        raise ValueError("Arquivo não contém CNPJs válidos")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Consulta Simples Nacional / MEI para lista de CNPJs"
    )

    parser.add_argument(
        "--file",
        help="Arquivo TXT contendo um CNPJ por linha",
        required=False
    )
    ...
    try:
        if args.file:
            cnpjs = ler_cnpjs_arquivo(args.file)
        else:
